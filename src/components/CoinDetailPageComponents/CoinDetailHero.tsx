            ...
            {coinDetailData && coinDetailData.market_data ? <div>
                ...
                {coinDetailData && coinDetailData.market_data && <>
                    ...
                    <nav className="container interFont lg:ml-20">
                        <ol className="list-reset py-4 pl-1 rounded flex bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-slate-400">
                            <li className="px-2"><a className="no-underline text-indigo">Cryptocurrencies</a></li>
                            <li>/</li>
                            <li className="px-2"><a className="no-underline text-indigo">Coins</a></li>
                            <li>/</li>
                            <li className="px-2 capitalize">{props.cryptoName}</li>
                            <li>/</li>
                            <li className="px-2"><Link href={"/"} className="no-underline text-indigo hover:underline">Home</Link></li>
                        </ol>
                    </nav>
                                                ...
                                                <SwiperSlide>
                                                    <div >
                                                        <div className="flex gap-6 flex-wrap">
                                                            <img src={coin.item.small} alt="" className='bg-white dark:bg-slate-900 rounded-full w-16 boxsh2' />
                                                            <p className='text-gray-400 dark:text-slate-400 font-semibold text-sm'> #{coin.item.market_cap_rank} </p>
                                                        </div>
                                                        <br />
                                                        <div className="flex flex-col gap-4">
                                                            <p className='text-base font-semibold'>{coin.item.name.substring(0, 8)}</p>
                                                            <p className='text-gray-400 dark:text-slate-400 font-normal text-sm'> {coin.item.symbol} </p>
                                                        </div>
                                                    </div>
                                                </SwiperSlide>
                ...
                {trendingCoinsData.length > 0 && <div className="coinDetailTagsHold ml-4 md:flex-col flex-row flex w37percent items-center lg:flex">
                    ...
                    <div className='flex items-center gap-4 justify-start w48percent w-full flex-wrap mt-3'>
                        {coinDetailData.categories && coinDetailData.categories[0] && <p className="bg-gray-200 dark:bg-slate-800 dark:text-slate-200 w-auto pl-3 pr-3 flex justify-center boxsh items-center rounded h-auto pt-2 pb-1"> {coinDetailData.categories[0]} </p>}
                        {coinDetailData.categories && coinDetailData.categories[1] && <p className="bg-gray-200 dark:bg-slate-800 dark:text-slate-200 w-auto pl-3 pr-3 flex justify-center boxsh items-center rounded h-auto pt-2 pb-1"> {coinDetailData.categories[1]} </p>}
                    </div>
                ...
                {trendingCoinsData.length > 0 && <div className="coinDetailTagsHold ml-4 md:flex-col flex-row flex w37percent items-center lg:flex">
                    ...
                    <div className='flex items-center gap-4 justify-start w48percent w-full flex-wrap mt-3'>
                        {coinDetailData.links && coinDetailData.links.homepage[0] && <Link href={`${coinDetailData.links.homepage[0]}`} target="_blank" className="bg-gray-200 dark:bg-slate-800 dark:text-slate-200 w-auto pl-3 pr-3 flex justify-center items-center rounded-full h-9 boxsh">  <IoHome /> </Link>}
                        {coinDetailData.links && coinDetailData.links.repos_url.github[0] && <Link href={`${coinDetailData.links.repos_url.github[0]}`} target='_blank' className="bg-gray-200 dark:bg-slate-800 dark:text-slate-200 w-auto pl-3 pr-3 flex justify-center items-center rounded-full h-9 boxsh"> <FaGithub /> </Link>}
                        {coinDetailData.links && coinDetailData.links.subreddit_url && <Link href={`${coinDetailData.links.subreddit_url}`} target='_blank' className="bg-gray-200 dark:bg-slate-800 dark:text-slate-200 w-auto pl-3 pr-3 flex justify-center items-center rounded-full h-9 boxsh"> <FaReddit /> </Link>}
                ...
                {trendingCoinsData.length > 0 && <div className="coindDetailStats1Hold pr-4 pl-4 hidden lg:flex items-center justify-evenly flex-wrap gap-10">
                    ...
                    <div className="coinDetailStat1Hold items-center flex flex-col gap-8">
                        ...
                        <div className="pl-3">
                            <div className="flex items-center gap-2">
                                <span className='text-2xl font-semibold text-gray-400 dark:text-slate-400 interFont'>$</span>
                    ...
                    <p className='bg-gray-100 dark:bg-slate-800 coinDetailDivider h-28'></p>
                    ...
                    <div className="coinDetailStat1Hold items-center flex flex-col gap-8">
                        ...
                        <div className="pl-3">
                            <div className="flex items-center gap-2">
                                ...
                                {coinDetailData.market_data && coinDetailData.market_data.circulating_supply && coinDetailData.symbol && <p className=' font-semibold statHeading interFont'>  {Math.floor(coinDetailData.market_data.circulating_supply).toLocaleString()}<span className=' text-gray-300 dark:text-slate-500 font-normal uppercase'> {coinDetailData.symbol} </span> </p>}
                            </div>
                        </div>
                    </div>
